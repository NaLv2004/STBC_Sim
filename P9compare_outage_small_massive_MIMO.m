...
fprintf('仿真开始，正在生成三张图表...\n');

%% ================== 场景 1: 归一化信道增益 PDF ==================
% 目标：展示随着天线数增加，信道能量如何集中到一个确定值
fprintf('正在运行场景 1: 信道增益 PDF...\n');
...
xlabel('Normalized Channel Gain (\|\bf{h}\|\|^2 / N_r)', 'FontSize', fontSize, 'FontName', fontName);
...
xlim([0.2 1.8]); 
ylim([0 8]); % 稍微限制Y轴以突出尖峰
hold off;

%% ================== 场景 2: 容量 CDF (可靠性对比) ==================
% 目标：展示 Massive MIMO 的容量曲线如何变成“阶跃函数”
fprintf('正在运行场景 2: 容量 CDF...\n');
...
for k = 1:length(configs)
    ...
    for i = 1:num_trials
        ...
        % 如果不除以 sqrt(Nr/Nt)，Massive MIMO 曲线会由阵列增益平移到非常右侧
        % 我们这里想看的是“斜率”的变化
        H_norm = H / sqrt(Nr/Nt); 
...
figure(3); set(gcf, 'Color', 'w', 'Position', [400, 550, 600, 400]);
